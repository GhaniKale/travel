            ...
            <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                <div className="lg:col-span-6 z-10">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <Link to="/packages" className="inline-flex justify-center items-center cta-gradient text-on-secondary px-8 py-4 rounded-xl font-headline font-bold text-base shadow-lg hover:shadow-secondary/20 transition-all active:scale-95 min-h-[48px] min-w-[150px]">
                            Book Your Trip
                        </Link>
                        <button
                            onClick={() => document.getElementById('packages')?.scrollIntoView({ behavior: 'smooth' })}
                            className="inline-flex justify-center items-center bg-surface-container-lowest text-primary px-8 py-4 rounded-xl font-headline font-bold text-base border-2 border-primary/10 hover:bg-primary/5 transition-all active:scale-95 min-h-[48px] min-w-[150px]"
                        >
                            View Packages
                        </button>
                    </div>
